<template>
	<Flexbox direction="column" gap="16" className="survey-slimming-page">
		...
		<Flexbox align="left" className="banner-container">
			...
			<Icon src="slimming.png" width="186" height="118" mode="heightFix" className="banner-image"></Icon>
		</Flexbox>
		<Flexbox direction="column" gap="16" className="survey-list">
			...
			<Flexbox direction="column" gap="16" className="survey-item" v-if="current === questions.length && loaded" @tap="navigateTo('/pagesC/tongue/diagnosis')">
				<Flexbox direction="column" gap="12" className="survey-message">
					<Flexbox direction="column" gap="8" className="full-width">
						<Typography fontSize="16">察颜观舌</Typography>
						<Flexbox gap="8" className="full-width">
							<view class="survey-description">
								<Typography color="gray2">用中医诊断的知识为您分析目前身体状况</Typography>
							</view>
		...
		<Flexbox align="center" direction="column" gap="8" className="analysis-container" v-if="current === questions.length && loaded && aiDocterResult.constitution_type">
...
export default {
	...
	computed: {
		...mapState(['userInfo', 'member', 'basicInfo', 'aiDocterResult'])
	},
	methods: {
		toggleInput(flag) {
			this.showInput = flag;
		...
		onAnalysis() {
			...
			request({
				...
				success: (res) => {
					if (res) {
						this.redirectTo('/pagesA/report/slimming');
					}
				},
				complete: () => {
					uni.hideLoading();
					...
					if (res) {
						...
						for (let key in res) {
							if (!['id', 'basicinfo'].includes(key)) {
								this.questions = this.questions.concat(this.transferData(res[key], key));
							}
						}
