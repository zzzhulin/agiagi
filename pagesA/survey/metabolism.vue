<template>
	<Flexbox direction="column" gap="16" className="survey-metabolism-page">
		...
		<Flexbox align="left" className="banner-container">
			...
			<Icon src="metabolism.png" width="186" height="108" mode="heightFix" className="banner-image"></Icon>
		</Flexbox>
	...
	methods: {
		...
		onAnalysis() {
			...
			request({
				...
				success: (res) => {
					if (res) {
						this.redirectTo(`/pagesA/report/metabolism?recordId=${res.id}&from=survey`);
					}
				},
				complete: () => {
					uni.hideLoading();
