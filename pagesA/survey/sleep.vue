<template>
	<Flexbox direction="column" gap="16" className="survey-sleep-page">
		<Navbar title="匹兹堡睡眠调查"></Navbar>
		<Flexbox align="left" className="banner-container">
			...
			<Icon src="sleep.png" width="186" height="108" mode="heightFix" className="banner-image"></Icon>
		</Flexbox>
	...
	methods: {
		...
		onAnalysis() {
			...
			request({
				...
				success: (res) => {
					if (res) {
						this.redirectTo(`/pagesA/report/sleep?recordId=${res.id}&from=survey`);
					}
				},
				complete: () => {
					uni.hideLoading();
