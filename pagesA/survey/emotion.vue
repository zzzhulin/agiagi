<template>
	<Flexbox direction="column" gap="16" className="survey-emotion-page">
		...
		<Flexbox align="left" className="banner-container">
			...
			<Icon src="emotion.png" width="186" height="108" mode="heightFix" className="banner-image"></Icon>
		</Flexbox>
	...
	methods: {
		...
		onAnalysis() {
			...
			request({
				...
				success: (res) => {
					if (res) {
						this.redirectTo(`/pagesA/report/emotion?recordId=${res.id}&from=survey`);
					}
				},
				complete: () => {
					uni.hideLoading();
